...
theory Virtual_Cut
  imports Source_History Replay Scope DBLog_Run_Substrate_Layer2
begin
...
text \<open>
  Public-to-core bridge: the public
  @{const virtual_cut_state} coincides with
  @{const core_virtual_cut_state} of
  @{theory DBLog_Virtual_Cuts.Virtual_Cut_Core} (identical bodies; distinct home
  theories). The theorem
  @{text wellformed_run_implies_virtual_cut} below uses it to rephrase
  the substrate locale's @{const core_virtual_cut_state} conclusion as the
  public @{const virtual_cut_state}.
\<close>

lemma virtual_cut_state_eq_core:
  "virtual_cut_state b0 \<sigma> K f H = core_virtual_cut_state b0 \<sigma> K f H"
  by (simp add: virtual_cut_state_def core_virtual_cut_state_def)
...
text \<open>
  ...
  @{const core_virtual_cut_state} conclusion to the public
  @{const virtual_cut_state} via @{thm [source] virtual_cut_state_eq_core}.
...
proof -
  ...
  have "core_virtual_cut_state b0 (canonical.clean_prefix_of R) (scope_of R)
                              (frontier_of R) H" .
  thus ?thesis
    by (simp add: canonical_clean_prefix_of_eq virtual_cut_state_eq_core)
qed
