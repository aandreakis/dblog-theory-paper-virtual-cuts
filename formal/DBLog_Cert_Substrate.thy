...
text \<open>
  ...
  The carrier-independent ``pure'' core (@{const core_virtual_cut_state}, the
...
text \<open>The central object, cert-side: on @{text "scope C"} the replay of
  ...
  The cert-accessor instance of the core @{const core_virtual_cut_state}.\<close>
...
where
  "virtual_cut b0 C H \<longleftrightarrow>
     core_virtual_cut_state b0 (clean_prefix C) (scope C) (frontier C) H"
...
lemma cert_run_coherent_virtual_cut_state_subst:
  assumes "cert_accessors_agree C R"
      and "core_virtual_cut_state b0 (clean_prefix_of R) (scope_of R)
                             (frontier_of R) H"
  shows   "core_virtual_cut_state b0 (clean_prefix C) (scope C)
                             (frontier C) H"
  using assms
...
proof -
  have scoped:
    ...
    using vc unfolding virtual_cut_def core_virtual_cut_state_def by simp
  have contained:
...
proof -
  ...
  have run_vcs:
    "core_virtual_cut_state b0 (clean_prefix_of R) (scope_of R)
                       (frontier_of R) H"
