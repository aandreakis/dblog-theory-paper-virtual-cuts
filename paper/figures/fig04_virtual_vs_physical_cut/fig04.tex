...
observed --- a monolithic vertical line. DBLog does \emph{not} produce
one; the values shown are the counterfactual ``if it did'' outcome on
$K$. \emph{Right:} a virtual cut requires only that replay-at-frontier
equal source-at-frontier on the chosen scope; the chunk reads (as in
