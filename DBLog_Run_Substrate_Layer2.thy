...
text \<open>
  ...
  carrier-independent state predicate @{const core_virtual_cut_state} for the
  concluding theorem.
...
text \<open>
  The run-side instance of @{const core_virtual_cut_state}: a wellformed DBLog
  ...
  coincides with the body-identical @{const core_virtual_cut_state} used
  here). Proved by
  unfolding @{const core_virtual_cut_state} and applying the locale per-key
...
theorem wellformed_run_implies_virtual_cut:
  ...
  shows
    "core_virtual_cut_state b0 (clean_prefix_of R) (scope_of R)
                       (frontier_of R) H"
  unfolding core_virtual_cut_state_def restrict_def
  by (intro ext)
